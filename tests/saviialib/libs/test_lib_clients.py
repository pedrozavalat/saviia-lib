...
from saviialib.libs.files_client import FilesClient, FilesClientInitArgs, ReadArgs, WriteArgs
from saviialib.libs.ftp_client import FTPClient, FtpClientInitArgs, FtpListFilesArgs, FtpReadFileArgs
from saviialib.libs.log_client import DebugArgs, ErrorArgs, InfoArgs, LogClient, LogClientArgs, LogStatus, WarningArgs
...
from saviialib.libs.sftp_client import DownloadfilesArgs, ListfilesArgs, SFTPClient, SFTPClientInitArgs
...
from saviialib.libs.weather_client import ForecastArgs, WeatherClient, WeatherClientInitArgs, WeatherMetric, WeatherQuery


def _ftp_config() -> FtpClientConfig:
    return FtpClientConfig(ftp_host="localhost", ftp_port=21, ftp_user="u", ftp_password="p")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            NotificationClient,
            NotificationClientInitArgs(client_name="discord_client", api_key="token", channel_id="ch"),
            "DiscordClient",
            "discord_client",
        ),
        (
            ...
            SharepointClient,
            SharepointClientInitArgs(config=_sharepoint_config(), client_name="sharepoint_rest_api"),
            "SharepointRestAPI",
            "sharepoint_rest_api",
        ),
        (
...
def test_should_initialize_supported_clients(monkeypatch, module_path, client_cls, args, client_attr, valid_name):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            SFTPClient,
            SFTPClientInitArgs(
                client_name="invalid", password="pw", username="u", ssh_key_path="", host="localhost", port=22
            ),
        ),
        (NotificationClient, NotificationClientInitArgs(client_name="invalid", api_key="token", channel_id="ch")),
        (SharepointClient, SharepointClientInitArgs(config=_sharepoint_config(), client_name="invalid")),
        (WeatherClient, WeatherClientInitArgs(client_name="invalid", latitude=1.0, longitude=2.0)),
    ],
)
...
async def test_should_delegate_email_send(monkeypatch):
    module = __import__("saviialib.libs.email_client.email_client", fromlist=["SmtpLibClient"])
    ...
    result = await client.send_email(SendEmailArgs("to@test.com", "subj", "body", "html"))
...
async def test_should_delegate_ftp_calls(monkeypatch):
    module = __import__("saviialib.libs.ftp_client.ftp_client", fromlist=["AioFTPClient"])
    fake = MagicMock(
    ...
    client = FTPClient(FtpClientInitArgs(config=_ftp_config(), client_name="aioftp_client"))
...
async def test_should_delegate_files_client_calls(monkeypatch):
    module = __import__("saviialib.libs.files_client.files_client", fromlist=["AioFilesClient"])
    fake = MagicMock(read=AsyncMock(return_value="ok"), write=AsyncMock(return_value=None))
...
async def test_should_delegate_directory_client_calls(monkeypatch):
    module = __import__("saviialib.libs.directory_client.directory_client", fromlist=["OsClient"])
    fake = MagicMock(
...
async def test_should_delegate_notification_client_calls(monkeypatch):
    module = __import__("saviialib.libs.notification_client.notification_client", fromlist=["DiscordClient"])
    fake = MagicMock(
    ...
    client = NotificationClient(NotificationClientInitArgs(client_name="discord_client", api_key="token", channel_id="ch"))

    await client.connect()
    await client.close()
    assert await client.notify(NotifyArgs(content="c", embeds=[], files=[])) == {"id": "1"}
    assert await client.list_notifications() == [{"id": "1"}]
    await client.react(ReactArgs(notification_id="1", emoji="✅"))
    assert await client.find_notification(FindNotificationArgs(notification_id="1")) == {"id": "1"}
    await client.update_notification(UpdateNotificationArgs(notification_id="1", new_content="n"))
...
async def test_should_delegate_sharepoint_client_calls(monkeypatch):
    module = __import__("saviialib.libs.sharepoint_client.sharepoint_client", fromlist=["SharepointRestAPI"])
    fake = MagicMock(
    ...
    client = SharepointClient(SharepointClientInitArgs(config=_sharepoint_config(), client_name="sharepoint_rest_api"))
    ...
    assert await client.list_files(SpListFilesArgs(folder_relative_url="/x")) == {"value": []}
    assert await client.list_folders(SpListFoldersArgs(folder_relative_url="/x")) == {"value": []}
    assert await client.upload_file(SpUploadFileArgs(folder_relative_url="/x", file_name="a", file_content=b"x")) == {"ok": True}
    assert await client.create_folder(SpCreateFolderArgs(folder_relative_url="/x")) == {"ok": True}


@pytest.mark.asyncio
async def test_should_delegate_weather_client_calls(monkeypatch):
    module = __import__("saviialib.libs.weather_client.weather_client", fromlist=["OpenmeteoClient"])
    fake = MagicMock(
        latitude=1.0,
        longitude=2.0,
        connect=AsyncMock(return_value=None),
    ...
    client = WeatherClient(WeatherClientInitArgs(client_name="open_meteo", latitude=1.0, longitude=2.0))
