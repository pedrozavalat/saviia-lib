...
class UploadBackupToSharepointUsecase:
    ...
    async def upload_and_log_progress_task(self, folder_name, file_name) -> dict:
        ...
        show_upload_result(uploaded, file_name)
        return {
    ...
    async def retry_upload_failed_files(self, results) -> None:
        ...
        tasks = []
        print(f"[BACKUP] Retrying upload for {len(failed_files)} failed files... 🚨")
        ...
        if success < 100.0:
            ...
            print("[BACKUP] All files uploaded successfully after retry.")
    ...
    async def execute(self):
        ...
        if self.total_files == 0:
            print(f"[BACKUP] {self.local_backup_source_path} has no files ⚠️")
        ...
        if success < 100.0:
            ...
            print(f"[BACKUP] Migration time: {backup_time:.2f} seconds ✨")
            return parse_execute_response(results)
